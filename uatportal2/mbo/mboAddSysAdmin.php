...
else
{
	...
	echo "<h1>Add System Administrator</h1><P>";
	...
	if ($passedErr != "0001") 
	{
		...
		if ($edits_ok == "Y")
		{
			...
			mboLogError ("audit: ADDING SYSTEM ADMIN: admsite=$admsite admuid=$admuid valinputadmid=$valinputadmid");
			...
			else
			{
				...
				. "'" . $valinputadmid . "'"
				. ",'" . $valinputadmsiteid . "'"
				. ',PASSWORD("' . $valsalt . strtoupper($valspin) . $valinputadmsiteid . strtoupper($valinputadmid) . '")'
				. ",'Y'"
				. ",'N'"
				. ", 3 "
				. ",'Y'"
	...
	{ 
		if ($passedErr == "0002")
		{
			...
			echo "<h3>Please give the following information to this System Administrator:</h3>";
	...
	echo '<P><form action="mboAddSysAdmin.php?err=';
	echo "$passedErr";
	echo '" method=POST>';

	if ($passedErr == "0002")
	{
		include("mboAddSysAdmGood.txt");
	}
	else
	{
		include("mboAddSysAdmAft.txt");
	}
