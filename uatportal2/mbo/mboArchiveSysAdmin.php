...
else
{
	...
	if ($filtuid != "")
	{
		...
		{
			mboLogError("mboArchiveSysAdmin.php: SQL Error: SQL=$sql");
		...
		if ($good_user == 'Y')
		{
			$filtparm = " WHERE user_id = '" . strtoupper($filtuid) . "'";
			$filtdescr = "Administrator";
			$filtactive = "Y";
		}
		else
		{
			mboLogError ("badinput: Invalid user id in querystring (" . $filtuid . ") in mboArchiveSysAdmin.php");
			mboFatalError();
		}
	}
	else
	{
		mboLogError ("badinput: Blank user id in querystring in mboArchiveSysAdmin.php");
		mboFatalError();
	}
	...
	{
		mboLogError("mboArchiveSysAdmin.php: SQL Error: SQL=$arcvsql");
		mboLogError ("DB error: " . mysql_error());
		mboFatalError();
	}
