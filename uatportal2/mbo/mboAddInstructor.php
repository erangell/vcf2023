...
else
{
	...
	if ($passedErr != "0001") 
	{
		...
		if ($edits_ok == "Y")
		{
			...
			else
			{
				...
				. "'" . $valinputadmid . "'"
				. ",'" . $admsite . "'"
				. ',PASSWORD("' . $valsalt . strtoupper($valspin) . $admsite . strtoupper($valinputadmid) . '")'
				. ",'Y'"
				. ",'N'"
				. ", 3 "
				. ",'Y'"
	...
	{ 
		if ($passedErr == "0002")
		{
			...
			echo "<h3>Please give the following information to this Instructor:</h3>";
	...
	echo '<P><form action="mboAddInstructor.php?err=';
	echo "$passedErr";
	echo '" method=POST>';

	if ($passedErr == "0002")
	{
		include("mboAddInstrGood.txt");
	}
	else
	{
		include("mboAddInstrAft.txt");
	}
