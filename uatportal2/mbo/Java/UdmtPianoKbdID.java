...
//
// Animate	=	'YES'=Run the animation routine - otherwise just display the note
//								OPTIONAL
//------------------------------------------------------------------------------
...
07/04/2010 - fix bug where C# generated in treble clef and Cb in bass clef
		   - also slowed down the game speed to allow user more time to move mouse

09/23/2011 - ver 0.96 slowed down the game by using _timeParm, added parm: Animate
...
//	private String strCorrect 			=	"Correct.";
	...
	private	final int	ROW1_Y					=	310;
	private	final	int	ROW2_Y					=	350;
	...
	private final int _timeParm = 400;
...
//------------------------------------------------------------------------------
	...
	public void init()
	{
		...
		//
		//Retrieve Parameter:  Animate
		//
		parmAnimate = getParameter("Animate");
		if ( parmAnimate != null ) {
			_Animate	 = ( parmAnimate.equals("YES") );
		}
		else {
			_Animate = false;
		}
...
//
			...
			beginButton.setEnabled(_Animate);
		if (_Animate)
		{
			add(beginButton);
		}
...
//
		...
		{
			takeExamButton.setBounds ( 180,ROW2_Y,BUTTON_WIDTH,BUTTON_HEIGHT);
		}
		add(takeExamButton);
		if ( examMode ) {
...
//
		...
		{
			exitButton.setBounds(300,ROW2_Y,BUTTON_WIDTH,BUTTON_HEIGHT);
		}
...
//			numTimesChecked++;
			...
			if ( examMode )	{
				...
				//2011-09-24: need to change this now that buttons removed
...
//------------------------------------------------------------------------------
	...
	public void GenerateQuestionDeck()
	{
		...
		for (q = 0; q < Q_DECK_MAX; q++) {
			...
			//2010-07-04: For Treble clef, no C#  For Bass Clef, no Cb
			int accid = 0;
...
//------------------------- IMPLEMENTATION OF RUNNABLE INTERFACE ---------------
    ...
    public void run() {
		...
		if (_NoteSlot > ENDING_NOTE_SLOT)
		{
			...
			{
				_NoteSlot = ENDING_NOTE_SLOT + ((STARTING_NOTE_SLOT - ENDING_NOTE_SLOT)/2);
			}
