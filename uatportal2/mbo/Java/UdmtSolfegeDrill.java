...
public class UdmtSolfegeDrill extends Applet
	implements MouseListener,  ActionListener, ItemListener, MetaEventListener
{
//------------------------------------------------------------------------------
// UdmtSolfegeDrill - Solfege drill and exam
//------------------------------------------------------------------------------
// PARAMS:
//
// solfegeType	= The type of exercises to present:
// Values: 
...
//------------------------------------------------------------------------------
// 11/07/2007: ver 0.11 - always display the notes
//------------------------------------------------------------------------------
// 12/29/2007: ver 0.12 - param to control enabling of Show Notes button: bShowNotesEnabled
//------------------------------------------------------------------------------
// 09/20/2011: ver 0.13 - don't allow student to use "Show Notes" button to avoid listening to the MIDI
//------------------------------------------------------------------------------

	private	UdmtNotationApi notation;
	private	UdmtSolfegeExercises exercises;
	...
	private String udmtAppletName			=	"UdmtSolfegeDrill";
	private String udmtAppletVersion		=	"0.13"	;
...
"Guitar Fret Noise","Breath Noise","Seashore","Bird","Telephone","Helicopter","Applause","Gunshot"
	...
	private String strDrillMode			=	"Solfege Drill";
	private String strExamMode			=	"Solfege Exam";
	private String strInstruct 			= 	"Identify the Solfege syllable for each note of the sequence.";
	...
	private int[] boxMidpt;				// solfege input boxes.
	...
	private String strShowCorrect			= 	"The correct solfege syllables are being displayed.";
	...
	int lBtn = 20;	// left coordinate of solfege button set
	...
	// amount to add to transposed sequence to get back to original - for calculating solfege syllable
	...
	private String parmShowNotesEnabled;
	private boolean bShowNotesEnabled = false;
//------------------------------------------------------------------------------
	...
	public void init()
	{
		...
		exercises = new UdmtSolfegeExercises();
		...
		bShowNotesEnabled = false;
		parmShowNotesEnabled = getParameter ("showNotesEnabled");
		if (parmShowNotesEnabled != null)
		{
		    if (parmShowNotesEnabled.equals("YES"))	
			{
				bShowNotesEnabled = true;
			}
		}
	    if (bShowNotesEnabled)
		{
			System.out.println ("bShowNotesEnabled=true");
		}
		else
		{
			System.out.println ("bShowNotesEnabled=false");
		}

		parmSolfType = getParameter ("solfegeType");
		if (parmSolfType == null)
		{
			showAbendScreen = 1;
			txtAbend = "Param: solfegeType is required";
			System.out.println ("The value for the parameter solfegeType must be one of the following:");
			...
			strDrillMode="Solfege Practice: Major";
			strExamMode="Solfege Exam: Major";
			...
			strDrillMode="Solfege Practice: Natural Minor";
			strExamMode="Solfege Exam: Natural Minor";
			...
			strDrillMode="Solfege Practice: Harmonic Minor";
			strExamMode="Solfege Exam: Harmonic Minor";
			...
			strDrillMode="Solfege Practice: Melodic Minor";
			strExamMode="Solfege Exam: Melodic Minor";
			...
			showAbendScreen = 1;
			txtAbend = "Param: solfegeType is invalid.";
			System.out.println ("The value for the parameter solfegeType must be one of the following:");
			System.out.println ("MajorScalar,MajorArp,NaturalMinorScalar,NaturalMinorArp,");
			System.out.println ("HarmonicMinorScalar,HarmonicMinorArp,MelodicMinorScalar,MelodicMinorArp");
		}
...
//		}
		...
		else
		{
			...
			if (examMode)
			{
				...
				String sExamKey = objEk.getExamKey("Solfege"+parmSolfType+"Pass");
		...
		if (examMode)
		{
			...
			else
			{
				...
				{
					UdmtExamKey objEk = new UdmtExamKey();
					String sExamKey = objEk.getExamKey("Solfege"+parmSolfType+"Fail");
		...
		if ( !examMode )	{
			...
			shownotesButton.setEnabled(bShowNotesEnabled);        
		...
		btnMe.setBounds ( xBtn,yBtn,wBtn+10,hBtn );
		add(btnMe);
		...
		// For solfege drill, popup menu always active.
		//xLeft = staffX + 50 ;
...
//	{
//		MenuItem mi ;
//		popup = new PopupMenu ( "Select Solfege Syllable" );
//
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		if (e.getActionCommand().equals ( "CHECK" ) )
		{
			...
			for (int q = 0 ; q < numMidiNotes ; q++ )
			{
				...
				if 	( 	(correctSolfVal == 10) 	// For Solfege exercises, all minor keys will use flats
					|| 	(correctSolfVal == 8)	// so we can get away with multiplying flat solfeges by -1
					||	(correctSolfVal == 6)
					||	(correctSolfVal == 3)
					||	(correctSolfVal == 1)
					)
			...
			else
			{
				if (!examMode)
				{
					//2011-09-20: Don't allow student to view notes - must listen to the MIDI
					//shownotesButton.setEnabled(true);
				}
			}
