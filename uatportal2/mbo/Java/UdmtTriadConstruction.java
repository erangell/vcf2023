//------------------------------------------------------------------------------
// UdmtTriadConstruction Applet -  Triad Identification Drill and Exam
//------------------------------------------------------------------------------
// PARAMS:
//
...
10/05/2008 - cloned from Triad ID drill ver 0.04
*/
...
public class UdmtTriadConstruction extends Applet implements ActionListener, ItemListener, MouseListener, MouseMotionListener
{
	...
	private String udmtAppletName			=	"UdmtTriadConstruction";
...
//	private UdmtMidiInQueue		midiIn;
	...
	private String strInstruct			= "Please construct the triad for the following scale degree:";
	...
	private				int 	correctRootNote;
	private				int 	correctThirdNote;
	private				int 	correctFifthNote;

	private				char 	correctRootAc;
	private				char 	correctThirdAc;
	private				char 	correctFifthAc;
	...
	private int userCursorBox;
	private int userCursorY;

	private int studentRootMidi = 0;
	private int studentRootRow = -1;
	private int studentRootAcVal = -3;
	private char studentRootAc = ' ';

	private int studentThirdMidi = 0;
	private int studentThirdRow = -1;
	private int studentThirdAcVal = -3;
	private char studentThirdAc = ' ';

	private int studentFifthMidi = 0;
	private int studentFifthRow = -1;
	private int studentFifthAcVal = -3;
	private char studentFifthAc = ' ';
...
//------------------------------------------------------------------------------
	...
	public void init()
	{
		...
		strDrillMode	="Triad Construction: ";
		strExamMode		="Triad Construction Exam: ";
			...
			txtError = "Parameter nextURL is required";
 			...
 			if (examMode)
 			{
 				UdmtExamKey objEk = new UdmtExamKey();
  				String sExamKey = objEk.getExamKey("TriadConstr"+parmContent+"Pass");
		...
		if ( examMode )	{
			...
			if ( parmDrillURL == null )	{
				...
				txtError				= "Parameter drillURL is required for Exam Mode";
 				...
 				{
 					UdmtExamKey objEk = new UdmtExamKey();
  					String sExamKey = objEk.getExamKey("TriadConstr"+parmContent+"Fail");
...
//------------------------------------------------------------------------------
	...
	private void DoJudging(int currentAnswer)
	{
		...
		if (_debug)
		{
			System.out.println("DoJudging: qAnswer="+qAnswer[currQues]+" currentAnswer="+currentAnswer);
		}

		if ((((rootMidi%12) == (studentRootMidi%12)) && ((thirdMidi%12) == (studentThirdMidi%12)) && ((fifthMidi%12) == (studentFifthMidi%12))) 
		&& (studentThirdMidi > studentRootMidi) && (studentFifthMidi > studentThirdMidi) 
		&& ((studentFifthMidi - studentRootMidi) <= 8)  // ensure triad is in root position, not an inversion
		&& ((studentRootRow - studentThirdRow) == 2) && ((studentThirdRow - studentFifthRow) == 2)
		)
		{					
			...
			if ((rootMidi%12) != (studentRootMidi%12)) 
			{
				if (_debug)
				{
					System.out.println("DoJudging: ROOT RED");
				}
				EraseAllNoteheadsInColumn(STAFF_COLUMN_ROOT);
				AddNoteheadToStaff(STAFF_COLUMN_ROOT,studentRootRow,studentRootAc,1,0);
			}
			if ((thirdMidi%12) != (studentThirdMidi%12)) 
			{
				if (_debug)
				{
					System.out.println("DoJudging: THIRD RED");
				}
				EraseAllNoteheadsInColumn(STAFF_COLUMN_THIRD);
				AddNoteheadToStaff(STAFF_COLUMN_THIRD,studentThirdRow,studentThirdAc,1,0);
			}
			if ((fifthMidi%12) != (studentFifthMidi%12)) 
			{
				if (_debug)
				{
					System.out.println("DoJudging: FIFTH RED");
				}
				EraseAllNoteheadsInColumn(STAFF_COLUMN_FIFTH);
				AddNoteheadToStaff(STAFF_COLUMN_FIFTH,studentFifthRow,studentFifthAc,1,0);
			}
...
//------------------------------------------------------------------------------
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (ac != '?')
		{
			if ((saveX >= boxLeft[0] ) && (saveX <= boxRight[0] ))
			{
				EraseAllNoteheadsInColumn(STAFF_COLUMN_ROOT);
				AddNoteheadToStaff(STAFF_COLUMN_ROOT, savecursorrow, ac,0);
				studentRootRow = savecursorrow; 
				studentRootAcVal = uaccval;
				studentRootAc = ac;
				int rootStaffMidi = RowToMidiNoteNum(studentRootRow, currentClef, currentKeySig);
				studentRootMidi = rootStaffMidi;
				if (studentRootAcVal == -3)
				{
					studentRootMidi = rootStaffMidi;	
				}
				else if (studentRootAcVal == +3) // natural
				{
					studentRootMidi = RowToMidiNoteNum(studentRootRow, currentClef, 0);
				}
				else if ((studentRootAcVal >= -2) && (studentRootAcVal <= +2))
				{
					studentRootMidi = RowToMidiNoteNum(studentRootRow, currentClef, 0) + studentRootAcVal;				
				}
				if (_debug)
				{
					System.out.println("rootStaffMidi="+rootStaffMidi+" studentRootRow="+studentRootRow+" studentRootAc="+studentRootAc+" studentRootMidi="+studentRootMidi);
				}
			}
			if ((saveX >= boxLeft[1] ) && (saveX <= boxRight[1] ))
			{			
				EraseAllNoteheadsInColumn(STAFF_COLUMN_THIRD);
				AddNoteheadToStaff(STAFF_COLUMN_THIRD, savecursorrow, ac,0);
				studentThirdRow = savecursorrow;
				studentThirdAcVal = uaccval; 
				studentThirdAc = ac;
				int thirdStaffMidi = RowToMidiNoteNum(studentThirdRow, currentClef, currentKeySig);
				studentThirdMidi = thirdStaffMidi;
				if (studentThirdAcVal == -3)
				{
					studentThirdMidi = thirdStaffMidi;	
				}
				else if (studentThirdAcVal == +3) // natural
				{
					studentThirdMidi = RowToMidiNoteNum(studentThirdRow, currentClef, 0);
				}
				else if ((studentThirdAcVal >= -2) && (studentThirdAcVal <= +2))
				{
					studentThirdMidi = RowToMidiNoteNum(studentThirdRow, currentClef, 0) + studentThirdAcVal;				
				}
				if (_debug)
				{
					System.out.println("thirdStaffMidi="+thirdStaffMidi+" studentThirdRow="+studentThirdRow+" studentThirdAc="+studentThirdAc+" studentThirdMidi="+studentThirdMidi);
				}
			}
			if ((saveX >= boxLeft[2] ) && (saveX <= boxRight[2] ))
			{
				EraseAllNoteheadsInColumn(STAFF_COLUMN_FIFTH);
				AddNoteheadToStaff(STAFF_COLUMN_FIFTH, savecursorrow, ac,0);
				studentFifthRow = savecursorrow; 
				studentFifthAcVal = uaccval;
				studentFifthAc = ac;
				int fifthStaffMidi = RowToMidiNoteNum(studentFifthRow, currentClef, currentKeySig);
				studentFifthMidi = fifthStaffMidi;
				if (studentFifthAcVal == -3)
				{
					studentFifthMidi = fifthStaffMidi;	
				}
				else if (studentFifthAcVal == +3) // natural
				{
					studentFifthMidi = RowToMidiNoteNum(studentFifthRow, currentClef, 0);
				}
				else if ((studentFifthAcVal >= -2) && (studentFifthAcVal <= +2))
				{
					studentFifthMidi = RowToMidiNoteNum(studentFifthRow, currentClef, 0) + studentFifthAcVal;				
				}
				if (_debug)
				{
					System.out.println("fifthStaffMidi="+fifthStaffMidi+" studentFifthRow="+studentFifthRow+" studentFifthAc="+studentFifthAc+" studentFifthMidi="+studentFifthMidi);
				}
			}

			if ((studentRootRow >= 0) && (studentThirdRow >= 0) && (studentFifthRow >= 0))
			{	
				checkButton.setEnabled(true);
				playTriadButton.setEnabled(true);
				playNotesButton.setEnabled(true);
				EraseAllNoteheadsInColumn(STAFF_COLUMN_TRIAD);
				AddNoteheadToStaff(STAFF_COLUMN_TRIAD, studentRootRow, studentRootAc,NudgeLeftRoot);
				AddNoteheadToStaff(STAFF_COLUMN_TRIAD, studentThirdRow, studentThirdAc,NudgeLeftThird);
				AddNoteheadToStaff(STAFF_COLUMN_TRIAD, studentFifthRow, studentFifthAc,NudgeLeftFifth);
			}
 		}
		...
		if ( e.getActionCommand().equals("PLAY TRIAD") )	{
			if ( !suppressingMIDI )	{
				...
				midiOut.addNoteOn(0, studentRootMidi);
				midiOut.addNoteOn(0, studentThirdMidi);
				midiOut.addNoteOn(0, studentFifthMidi);	
				midiOut.addNoteOff(70, studentRootMidi);
				midiOut.addNoteOff(70, studentThirdMidi);
				midiOut.addNoteOff(70, studentFifthMidi);
		...
		if ( e.getActionCommand().equals("PLAY NOTES") )	{
			if ( !suppressingMIDI )	{
				...
				midiOut.addProgChg(0, userInstrument );
				midiOut.addNoteOn(0, studentRootMidi);
				midiOut.addNoteOff(22, studentRootMidi);
				midiOut.addNoteOn(24, studentThirdMidi);
				midiOut.addNoteOff(46, studentThirdMidi);
				midiOut.addNoteOn(48, studentFifthMidi);
				midiOut.addNoteOff(70, studentFifthMidi);
				midiOut.addEndOfTrack(72);
				playMIDI( 120,0 );
			}
		...
		if ( e.getActionCommand().equals("SHOW CORRECT") )	{
			...
			AddNoteheadToStaff(STAFF_COLUMN_ROOT, correctRootNote, correctRootAc,0);
			AddNoteheadToStaff(STAFF_COLUMN_THIRD, correctThirdNote, correctThirdAc,0);
			AddNoteheadToStaff(STAFF_COLUMN_FIFTH, correctFifthNote, correctFifthAc,0);

			AddNoteheadToStaff(STAFF_COLUMN_TRIAD, correctRootNote, correctRootAc,NudgeLeftRoot);
			AddNoteheadToStaff(STAFF_COLUMN_TRIAD, correctThirdNote, correctThirdAc,NudgeLeftThird);
			AddNoteheadToStaff(STAFF_COLUMN_TRIAD, correctFifthNote, correctFifthAc,NudgeLeftFifth);

			studentRootRow = correctRootNote;
			studentRootAc = correctRootAc;
			studentRootMidi = rootMidi;

			studentThirdRow = correctThirdNote;
			studentThirdAc = correctThirdAc;
			studentThirdMidi = thirdMidi;

			studentFifthRow = correctFifthNote;
			studentFifthAc = correctFifthAc;
			studentFifthMidi = fifthMidi;
	...
	private String DisplayAnswer(int answer)
	{
		...
		return GetRomanNumeral(degree+1,capital); //+" "+qual;
	}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
	...
	{
		AddNoteheadToStaff(col,  row, accid, 0, accidNudge );
	}
	public void AddNoteheadToStaff(int col, int row, char accid, int red, int accidNudge )
	{
		System.out.println("AddNoteheadToStaff: "+col+","+row+","+accid+","+red);
		int	i		 = 0;
		int slot = -1;
...
//------------------------------------------------------------------------------
	...
	public void PresentCurrentQuestion(int q)
	{
		...
		if (qual == 1) 
			{ quality = 'm'; qualstr = "min"; kapital = 0; }
		else 
		{	if (qual == 2) { quality = '+'; qualstr = "aug"; kapital = 1; }
			else { 
				if (qual == 3) { quality = '-'; qualstr = "dim"; kapital = 0; } 
			}
		}

		currentStudentRomanNumeral = GetRomanNumeral((qAnswer[q])%10,kapital);
		currentStudentChordQuality = qualstr;
		

		if (_debug)
		{ 		
			System.out.println("PresentCurrentQuestion: currentStudentRomanNumeral="+currentStudentRomanNumeral+"startnote="+startNote+" qual="+qual+" quality="+quality);
		}
		...
		correctThirdAc = staffinfo3ac;
		correctThirdNote = startNote - 2;
		...
		correctFifthAc = staffinfo5ac;
		correctFifthNote = startNote - 4;
...
//------------------------------------------------------------------------------
	...
	private void displayScore(Graphics g)
	{
		...
		//centerString (g, strInstruct, txtFont, Y_STUDENT_CHORD);
		g.drawString (strInstruct, 60, Y_STUDENT_CHORD);
		
		centerString(g, strExamFeedback1, txtFont, EXAM_FB_LINE_1);
		centerString(g, strExamFeedback2, txtFont, EXAM_FB_LINE_2);

		String currentStudentChord = currentStudentRomanNumeral; // +" "+ currentStudentChordQuality;

		g.drawString(currentStudentChord, X_STUDENT_CHORD-10, Y_STUDENT_CHORD);
		g.drawRect(X_STUDENT_CHORD - 15 , Y_STUDENT_CHORD - 20, 125, 30);
...
//------------------------------------------------------------------------------
	...
	public void drawScreen(Graphics g)
	{
		...
		{
			int extra = 15;
			boxLeft[boxnum] = staffX + staffNoteXPos[i-1] + 7 - extra;
			boxTop[boxnum] = staffY + staffNoteYPos[0] - 25;
			boxWidth[boxnum] = staffNoteXPos[i+1] - staffNoteXPos[i-1] + extra;
			boxRight[boxnum] = boxLeft[boxnum] + boxWidth[boxnum];
			boxHeight[boxnum] = 175;
			boxBottom[boxnum] = boxTop[boxnum] + boxHeight[boxnum];
			g.drawRect(boxLeft[boxnum], boxTop[boxnum], boxWidth[boxnum], boxHeight[boxnum]);
			boxnum++;
...
//------------------------------------------------------------------------------
	public String GetRomanNumeral(int degree, int capital)
	{
		if (_debug)
		{
			System.out.println("GetRomanNumeral: degree="+degree);
		}
		...
		if (capital > 0)
		{
			switch (degree)
			{
				...
				case 1: {retval = "Tonic"; break;}
				case 2: {retval = "Supertonic"; break;}
				case 3: {retval = "Mediant"; break;}
				case 4: {retval = "Subdominant"; break;}
				case 5: {retval = "Dominant"; break;}
				case 6: {retval = "Submediant"; break;}
				case 7: {
						if (contentMode == NATURAL_MINOR)
						{	retval = "Subtonic";
						}
						 else
						{	retval = "Leading Tone";
...
//------------------------------------------------------------------------------
	...
	public void mouseMoved(MouseEvent e)
	{
		...
		//strFeedback = "mouseMoved: "+xcoord+","+ycoord;
