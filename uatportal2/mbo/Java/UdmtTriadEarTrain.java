//------------------------------------------------------------------------------
// UdmtTriadEarTrain Applet -  Triad Identification Drill and Exam
//------------------------------------------------------------------------------
// PARAMS:
//
...
public class UdmtTriadEarTrain extends Applet implements ActionListener, ItemListener, MouseListener, MouseMotionListener
{
	...
	private String udmtAppletName			=	"UdmtTriadEarTrain";
...
//	private	Button Roman7m;
	...
	private boolean _ShowTheNotes = false;
...
//------------------------------------------------------------------------------
	...
	public void init()
	{
		...
		strDrillMode	="Triad Ear Training";
		strExamMode		="Triad Ear Training Exam: ";
			...
			txtError = "Parameter nextURL is required";
 			...
 			if (examMode)
 			{
 				UdmtExamKey objEk = new UdmtExamKey();
  				String sExamKey = objEk.getExamKey("TriadEarTrainPass");
		...
		if ( examMode )	{
			...
			if ( parmDrillURL == null )	{
				...
				txtError				= "Parameter drillURL is required for Exam Mode";
 				...
 				{
 					UdmtExamKey objEk = new UdmtExamKey();
  					String sExamKey = objEk.getExamKey("TriadEarTrainFail");
...
//		add(Roman7);
		...
		btnMajTriad.setBounds(PITCH_BOX_X1+3*PITCH_BOX_SEP, PITCH_BOX_Y2, PITCH_BOX_WIDTH, PITCH_BOX_HEIGHT);
		add(btnMajTriad);
		...
		btnMinTriad.setBounds(PITCH_BOX_X1+4*PITCH_BOX_SEP,  PITCH_BOX_Y2, PITCH_BOX_WIDTH, PITCH_BOX_HEIGHT);
		add(btnMinTriad);

		btnAugTriad = new Button("+");
		btnAugTriad.addActionListener(this);
		btnAugTriad.setEnabled(true);
		btnAugTriad.setBounds(PITCH_BOX_X1+5*PITCH_BOX_SEP, PITCH_BOX_Y2, PITCH_BOX_WIDTH, PITCH_BOX_HEIGHT);
		add(btnAugTriad);
		...
		btnDimTriad.setBounds(PITCH_BOX_X1+6*PITCH_BOX_SEP,  PITCH_BOX_Y2, PITCH_BOX_WIDTH, PITCH_BOX_HEIGHT);
		add(btnDimTriad);
...
//------------------------------------------------------------------------------
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if ( e.getActionCommand().equals("m") ) {
			...
			csqual = 1;
			currentStudentRomanNumeral = "i";
			currentStudentChordQuality = "min";
			repaint();
		}
...
//------------------------------------------------------------------------------
	...
	private void displayScore(Graphics g)
	{
		...
		//}
		String currentStudentChord = dispCurrentStudentChordQuality;

		g.drawString(currentStudentChord, X_STUDENT_CHORD-50, Y_STUDENT_CHORD);
		g.drawRect(X_STUDENT_CHORD - 60 , Y_STUDENT_CHORD - 20, 110, 30);
