...
//------------------------------------------------------------------------------
// UdmtTsigNotationApi - Interprets command strings of notation rendering commands
...
//------------------------------------------------------------------------------
// Version 0.05 = version originally used for Solfege Drill
// Version 0.06 = corrected bugs with dotted notes and midiTimes array
// Version 0.07 = corrected MIDI playback for rests
// Version 0.08 = enhanced Time Signature logic //------------------------------------------------------------------------------


public class UdmtTsigNotationApi extends Object
{
	private String UdmtAppletVersion = "0.08";
	...
	int muNum9, muCut, muAccent, muMetronome, muTriplet;
	...
	private int beamDivision=1;
	private int drawTriplet = 0;
	...
	private int durWhole = 48, durHalf = 24, durQtr = 12, dur8th = 6, dur16th= 3;
...
//------------------------------------------------------------------------------
	...
	public int getTsigNumerator()
	{
		return tsigNum;
	}
	public int getTsigDenominator()
	{
		int iRet;
		if (tsigDen == 100)	
		{
			iRet = tsigNum; // cut or common time = 2/2 or 4/4
		}
		else
		{
			iRet = tsigDen;
...
//----------------------------------------------------------------------------
	...
	private void modeTime ( char c)
	{
		...
		if ((tsigNum == 0) && (tsigDen == 0))
		{
			switch (c)
			{
				...
				case '5': {  tsigNum = 5; break;  }
				case '6': {  tsigNum = 6; break;  }
				case '7': {  tsigNum = 7; break;  }
				case '8': {  tsigNum = 8; break;  }
				case '9': {  tsigNum = 9; break;  }
				case 'A': {  tsigNum = 10; break; }	// use hex chars for numbers > 9
				case 'B': {  tsigNum = 11; break; }	// use hex chars for numbers > 9
				case 'C': {  tsigNum = 12; break; }	// use hex chars for numbers > 9
				case 'D': {  tsigNum = 13; break; }	// use hex chars for numbers > 9
				case 'E': {  tsigNum = 14; break; }	// use hex chars for numbers > 9
				case 'F': {  tsigNum = 15; break; }	// use hex chars for numbers > 9
	...
	private void renderTimeSig (int num, int den)
	{
		...
		else 
		{
			switch (num)
			{
				...
				case 7: {  
					add2staff (muNum7, 0, 0, 0);			
					break;
				}
				case 8: {  
					add2staff (muNum8, 0, 0, 0);			
					break;
				}
				case 9: {  
					add2staff (muNum9, 0, 0, 0);			
					break;
				}
				case 10: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum0, +6, 0, 0);
					break;
				}
				case 11: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum1, +6, 0, 0);
					break;
				}

				case 12: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum2, +6, 0, 0);
					break;
				}
				case 13: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum3, +6, 0, 0);
					break;
				}
				case 14: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum4, +6, 0, 0);
					break;
				}
				case 15: {
					add2staff (muNum1, -7, 0, 0);
					add2staff (muNum5, +6, 0, 0);
					break;
				}
			...
			{
				case 1: { 
					add2staff (muNum1, 0, 18, 25);			
					break;
				}
				case 2: { 
					if ((num >=4) && (num <= 5)) { adj = 1; }
					else if (num >= 6 ) { adj = 2; }
					add2staff (muNum2, adj, 18, 25);			
					break;
				}
				case 4: { 
					if ((num >=2) && (num <= 3)) { adj = -1; }
					else if (num == 6 ) { adj = 1; }
					add2staff (muNum4, adj, 18, 25);			
					break;
				}
				case 8: { 
					if ((num >=2) && (num <= 3)) { adj = -2; }
					else if (num == 6 ) { adj = 1; }
					else if (num == 7 ) { adj = 2; }
					add2staff (muNum8, adj, 18, 25);			
					break;
				}
				case 16: { 
					add2staff (muNum1, -7, 18, 25);	
					add2staff (muNum6, -20, 18, 25);	
					break;
				}
	...
	private void modeNote ( char c)
	{
		...
		else if (c == '`')	{	// put ` before the triplet note that will get the number 3 drawn above it
			drawTriplet = 1;
		}
		else if (c == '^')	{
		}
						...
						, int xNudge, int yNudge, int lowerLegerNudge)
	{
		...
		if (numDots > 0)
		{
			...
			if (currStemDir == +1)	{
				//System.out.println ("Stem is up.  numDots="+numDots);
				yadjust = staffNoteYPos[ypos] - 20 + yShift + extraNudge ;
				if (numDots == 1) {
					//System.out.println ("Adding one dot to staff.");
					add2staff ( muDot, 0 + xNudge - 10, yadjust + yNudge + extraNudge , 2);
				}
				else if (numDots == 2) {
					//System.out.println ("Adding two dots to staff.");
					add2staff ( muDblDot, 0 + xNudge - 10, yadjust + yNudge + extraNudge , 7);
				}
			}
			else if (currStemDir == -1)	{
				//System.out.println ("Stem is down. numDots="+numDots);
		...
		{
			drawTriplet = 0;
			add2staff ( muTriplet, 0 + xNudge - 27, yadjust + yNudge + extraNudge - 30 , 2);
		}
...
//------------------------------------------------------------------------------
	...
	private void addRestEvent ( long mtime, int stsbyte, int chnl, int notenum, int vel)
	{
		//System.out.println ("addRestEvent: mtime="+mtime+" stsbyte="+stsbyte+" chnl="+chnl+" notenum="+notenum+" vel="+vel);
		
		try {		
			msg = new ShortMessage();
			msg.setMessage(stsbyte, chnl, notenum, 0);
...
//------------------------------------------------------------------------------
	...
	private void addToSeq (int oct, char noteLetter, int accid, int dur, int dots )
	{
		...
		if (beamDivision == 3)
		{
			mididur = dur * 2 / 3;
			beamDivision = 1;
			//System.out.println ("Triplet: dur="+dur+" mididur="+mididur);
		}

		//System.out.println ("dots="+dots+" dur="+dur+" mididur="+mididur);
		...
		if (!suppressingMIDI)
		{
			...
			addNoteEvent ( (long)(miditime + mididur) , 144, 1, midinotenum, 0 );
			
			midiNotes [ midiNoteCount ] = midinotenum;
			midiTimes [ midiNoteCount ] = (long)(miditime + mididur);
			midiNoteCount++;
		}
		miditime += mididur;
		// 4/20/04: There was a bug in previous versions where it added "dur" instead of "mididur"
		// which resulted in incorrect sounding dotted notes.
	}
...
//------------------------------------------------------------------------------
	...
	private void addRestToSeq (int dur, int dots)
	{
		...
		if (beamDivision == 3)
		{
			mididur = dur * 2 / 3;
			beamDivision = 1;
			//System.out.println ("Triplet: mididur="+mididur);
		}
		...
		if (!suppressingMIDI)
		{
			addNoteEvent ( miditime, 144, 1, 0, 0);
			addNoteEvent ( (long)(miditime + mididur) , 144, 1, 0, 0 );
			
			midiNotes [ midiNoteCount ] = 0;
			midiTimes [ midiNoteCount ] = (long)(miditime + mididur);
			midiNoteCount++;
		}
		miditime += mididur;
