...
else
{
	...
	if ($filtuid != "")
	{
		...
		{
			mboLogError("mboEditAdmin.php: SQL Error: SQL=$sql");
		...
		{
			mboLogError ("badinput: Invalid user id in querystring (" . $filtuid . ") in mboEditAdmin.php");
			mboFatalError();
		}
	}
	else
	{
		mboLogError ("badinput: Blank user id in querystring in mboEditAdmin.php");
		mboFatalError();
	}
	...
	if ($passedErr == "0001")
	{
		...
		if ($numrows == 0)
		{	
			mboLogError ("mboEditAdmin.php: No data found for user");
			mboFatalError();
		}
		else
		{
	...
	echo '<P><form action="mboEditAdmin.php?err=';
	echo "$passedErr";
	echo '&ui=';
	echo $filtuid;
	echo '" method=POST>';
	...
	if ($passedErr == "0002")
	{
		include("mboEditAdminGood.txt");
	}
	else
	{
		include("mboEditAdminAft.txt");
	}
