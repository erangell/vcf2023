...
else
{
	...
	include("mboHeader.txt");
        echo "<h1>Edit System Adminstrator</h1><P>";
	...
	if ($filtuid != "")
	{
		...
		{
			mboLogError("mboEditSysAdmin.php: SQL Error: SQL=$sql");
		...
		{
			mboLogError ("badinput: Invalid user id in querystring (" . $filtuid . ") in mboEditSysAdmin.php");
			mboFatalError();
		}
	}
	else
	{
		mboLogError ("badinput: Blank user id in querystring in mboEditSysAdmin.php");
		mboFatalError();
	}
	...
	if ($passedErr == "0001")
	{
		...
		if ($numrows == 0)
		{	
			mboLogError ("mboEditSysAdmin.php: No data found for user");
			mboFatalError();
		}
		else
		{
	...
	echo '<P><form action="mboEditSysAdmin.php?err=';
	echo "$passedErr";
	echo '&ui=';
	echo $filtuid;
	echo '" method=POST>';
	...
	if ($passedErr == "0002")
	{
		include("mboEditSysAdminGood.txt");
	}
	else
	{
		include("mboEditSysAdminAft.txt");
	}
