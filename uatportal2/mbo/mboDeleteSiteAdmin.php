...
else
{
	...
	if ($filtuid != "")
	{
		...
		{
			mboLogError ("badinput: Invalid user id in querystring (" . $filtuid . ") in mboDeleteSiteAdmin.php");
			mboFatalError();
		}
	}
	else
	{
		mboLogError ("badinput: Blank user id in querystring in mboDeleteSiteAdmin.php");
		mboFatalError();
	}
	...
	if (!($results = mysql_query($sql)))
	{
		mboLogError("mboDeleteSiteAdmin.php: SQL Error: SQL=$sql");
		mboLogError ("DB error: " . mysql_error());
		mboFatalError();
	}
	...
	if ($numrows == 0)
	{	
		mboLogError ("mboDeleteSiteAdmin.php: No data found for user");
		mboFatalError();
	}
	else
	{
		...
		include ("mboDelSiteAdminAft.txt");
	}

	echo '<p>';
	echo '<li><a href="mboArchiveSiteAdmin.php?ui=' . $filtuid . '">Delete this Administrator</a>';
	echo '<p>';
	echo '<li><a href="mboMaintSiteAdmin.php">Return to Site Admin Maintenance</a>';
